...
// For Node user
// 連線其他節點取得區塊練
    public static ArrayList<Block> Connection_to_Node(String host, int port, String strBlockchain, int chainSize) throws IOException, InterruptedException {
        String debug="";
        String command = "ask-blockchain";
        ...
        try {
            ...
            SocketWrite(String.valueOf(chainSize), socket);
            Thread.sleep(TIME_DELAY);

            // get response
            res = SocketRead(socket);
            if("same length".equals(res)){ // 一樣長度
                System.out.println("Remote Node have same length chain");
                socket.close();
                return null;
            }
            else if("Ur chain longer".equals(res)){
                ...
                // Send blockchain
                Thread.sleep(TIME_DELAY);
                SocketWrite(strBlockchain, socket);
                ...
                socket.close(); // return the same chain back
                return UserFunctions.Convert2Blockchain(strBlockchain,chainSize);
/*
                // 更 新 master
                master = null;
                // 取消排程
 ...
 */
            ...
            // get new blockchain
            strBlockchain = SocketRead(socket);

            // get new blockchain size
            chainSize = Integer.parseInt(SocketRead(socket));
            System.out.println("new size: "+chainSize);

            System.out.println("get new chain\n");
            UserFunctions.printOutBlockchain(strBlockchain,chainSize);
